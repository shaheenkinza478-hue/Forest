...
export default function TermsPage() {
  ...
  const openModal = (title, desc, type = 'Term', imageIndex = 0) => {
    setModalContent({
  ...
  return (
    <div className="bg-white text-gray-800">
      ...
      <section
        ...
        style={{
          backgroundImage: `url(' ./forset6.jpg')`,
        }}
      >
        ...
        <div ref={ref1} className={`relative z-10 text-center text-white max-w-5xl px-4 sm:px-6 -mt-20 ${revealClass(vis1)}`}>
          ...
          <p className="mt-4 text-base sm:text-lg md:text-xl text-green-100 max-w-3xl mx-auto drop-shadow">
            Please read these terms carefully — by exploring our forest, you agree to walk gently.
          </p>
        </div>
      </section>

      {/* 2. INTRODUCTION – کلک کرنے پر موڈل */}
            ...
            {[
              {
                icon: BookOpen,
                title: 'Acceptance',
                desc: 'By using this website, you accept these terms in full.',
              },
              {
                icon: FileText,
                title: 'Updates',
                desc: 'We may revise these terms at any time. Continued use means you accept changes.',
              },
              {
                icon: HelpCircle,
                title: 'Questions?',
                desc: 'If anything is unclear, reach out before continuing.',
              },
            ].map((item, i) => (
              <div
                key={i}
                onClick={() => openModal(item.title, item.desc, 'Introduction', i)}
            ...
            {[
              {
                ...
                shortDesc: 'All content is for personal, non‑commercial use only.',
                longDesc: 'Text, images, and graphics on this website are protected by copyright. You may not reproduce, distribute, or use them commercially without written permission. Sharing with credit is welcome!',
              },
              {
                title: 'User Conduct',
                shortDesc: 'Respect the forest and fellow explorers.',
                longDesc: 'You agree not to post harmful, offensive, or illegal content. We reserve the right to remove any user submissions that violate these standards.',
              },
              {
                title: 'Account Responsibility',
                shortDesc: 'You are responsible for keeping your credentials safe.',
                longDesc: 'If you create an account, you must provide accurate information and keep your password confidential. Notify us immediately of any unauthorized use.',
              },
            ].map((item, i) => (
              <div
                key={i}
                onClick={() => openModal(item.title, item.longDesc, 'Term Condition', i + 2)}
              ...
              >
                <div className="shrink-0">
                  <Shield className="text-green-600 w-6 h-6 group-hover:scale-110 transition-transform" />
                </div>
                <div>
      ...
      {/* 4. LIMITATION OF LIABILITY – اعداد و شمار */}
      <section className="py-16 bg-green-800 text-white">
        <div ref={ref4} className={`max-w-7xl mx-auto px-4 sm:px-6 grid grid-cols-2 md:grid-cols-4 gap-8 text-center ${revealClass(vis4)}`}>
          {[
            { number: '∞', label: 'Good Faith' },
            { number: '0', label: 'Tolerance for Harm' },
      ...
      >
        ...
        <div ref={ref5} className={`relative max-w-4xl mx-auto px-4 sm:px-6 text-center text-white ${revealClass(vis5)}`}>
          <Star className="mx-auto text-yellow-400 w-12 h-12 mb-6" />
          <p className="text-2xl md:text-4xl italic font-light leading-relaxed">
            “Clear, fair terms — I never felt lost in legalese. A pleasure to explore this forest.”
          </p>
          <p className="mt-6 text-green-200 font-medium">— Marcus F.</p>
        </div>
      </section>

      {/* 6. STILL HAVE QUESTIONS? */}
      <section className="py-20 bg-green-50">
        <div ref={ref6} className={`max-w-3xl mx-auto text-center px-4 ${revealClass(vis6)}`}>
          <Award className="mx-auto text-green-700 w-12 h-12 mb-4" />
          <h2 className="text-3xl sm:text-4xl font-bold text-green-900 mb-6">
            Something Unclear?
          </h2>
          <p className="text-lg text-gray-700 mb-8">
            Our rangers are happy to explain anything. Just drop us a message.
          </p>
      ...
      >
        ...
        <div ref={ref7} className={`relative max-w-4xl mx-auto text-center text-white px-4 ${revealClass(vis7)}`}>
          <Leaf className="mx-auto w-12 h-12 mb-4 text-green-200" />
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-6">
            Ready to Walk the Wild Path?
          </h2>
          <p className="text-green-100 text-lg mb-8 max-w-xl mx-auto">
            Now that you know the rules, step into the forest with confidence.
          </p>
          <Link
            href={ROUTES.EXPLORE}
